...
def test_dashboard():
    ...
    # net-worth breakdown on the latest snapshot (label "a" -> other)
    assert payload["net_worth_breakdown"]["assets"]["other"] == 100.0
    assert payload["net_worth_breakdown"]["total_assets"] == 100.0
    assert payload["net_worth_breakdown"]["net_worth"] == 70.0
...
def test_net_worth_breakdown():
    s = snapshot_net_worth(
        "2026-09-01",
        {"demo-savings": 10500.00, "demo-brokerage": 47000.00, "demo-401k": 25000.00},
        {"demo-credit-card": 1500.00, "demo-student-loan": 9100.00, "demo-mortgage": 180000.00},
    )
    bd = net_worth_breakdown(s)
    assert bd["assets"]["cash"] == Decimal("10500.00")
    assert bd["assets"]["investments"] == Decimal("47000.00")
    assert bd["assets"]["retirement"] == Decimal("25000.00")
    assert bd["liabilities"]["credit_cards"] == Decimal("1500.00")
    assert bd["liabilities"]["student_loans"] == Decimal("9100.00")
    assert bd["liabilities"]["mortgage"] == Decimal("180000.00")
    assert bd["total_assets"] == Decimal("82500.00")
    assert bd["total_liabilities"] == Decimal("190600.00")
    assert bd["net_worth"] == Decimal("-108100.00")
    # shares sum to ~100 on each side
    assert abs(sum(bd["asset_share_pct"].values()) - 100.0) < 1e-9
    assert abs(sum(bd["liability_share_pct"].values()) - 100.0) < 1e-9
    # "credit card" must not fall into auto_loans ("car" is a substring of "card")
    assert bd["liabilities"]["auto_loans"] == Decimal("0")
    # classifier unit checks
    assert classify_asset("demo-401k") == "retirement"
    assert classify_asset("weird-account") == "other"
    assert classify_liability("demo-auto-loan") == "auto_loans"
    # empty sides -> 0.0 shares, no ZeroDivisionError
    empty = net_worth_breakdown(snapshot_net_worth("2026-09-01", {}, {}))
    assert empty["asset_share_pct"]["cash"] == 0.0
    assert empty["liability_share_pct"]["other"] == 0.0
    assert empty["net_worth"] == Decimal("0")
