...
bd = net_worth_breakdown(latest)
print(f"net worth breakdown ({latest.as_of}): net={bd['net_worth']}")
for cls, amt in bd["assets"].items():
    if amt:
        print(f"  asset {cls}: {amt} ({bd['asset_share_pct'][cls]:.1f}%)")
for cls, amt in bd["liabilities"].items():
    if amt:
        print(f"  liability {cls}: {amt} ({bd['liability_share_pct'][cls]:.1f}%)")
